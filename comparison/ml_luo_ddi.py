...
def get_features(data,fp_df,prodes_df,use_pro):
    drug_features = pd.merge(data,fp_df,how='left',left_on='head',right_on='drug_id').iloc[:,4:105].values
    pro_features = pd.merge(data,prodes_df,how='left',left_on='tail',right_on='drug_id').iloc[:,4:105].values
    if use_pro:
...
def train(i):
    train = pd.read_csv('./data/luo/data_folds/warm_start_1_1_ddi/train_fold_'+str(i+1)+'.csv')
    test = pd.read_csv('./data/luo/data_folds/warm_start_1_1_ddi/test_fold_'+str(i+1)+'.csv')
    ...
    train_des = get_features(re_train_all,fp_df,fp_df,use_pro=True)
    test_des = get_features(re_test_all,fp_df,fp_df,use_pro=True)
