...
def get_features(data,fp_df,prodes_df,use_pro):
    ...
    pro = pd.merge(data,prodes_df,how='left',left_on='tail',right_on='pro_id')
...
def train(i):
    train = pd.read_csv('./data/BioKG/data_folds/warm_start_1_10/train_fold_'+str(i+1)+'.csv')
    test = pd.read_csv('./data/BioKG/data_folds/warm_start_1_10/test_fold_'+str(i+1)+'.csv')
    ...
    #rf
    clf_svm = svm.SVC(C=1.0, kernel='rbf', degree=3, gamma='auto', coef0=0.0, shrinking=True, probability=False, tol=0.001, cache_size=200, class_weight=None, verbose=False, max_iter=-1, decision_function_shape=None,random_state=None)
