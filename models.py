...
class KGE_UNIT(nn.Sequential):
    def __init__(self, **config):
        ...
        #The structure of CNN for heterogeneous features of ddi
        ...
        #The structure of task-interacted attention block
        ...
        #The structure of task-aware attention block
        ...
        #The structure of task-specific head
    ...
    def forward(self,hf_dti,sf_dti_drug,sf_dti_protein,hf_ddi,sf_ddi_d1,sf_ddi_d2):
        ##For DTI:
        #unsqueeze of drug_feats
        drug_feats = sf_dti_drug.unsqueeze(2).repeat(1,1,self.dim_ ).cuda().float()
        #The forward of CNN of drug,to learn the structure features of drug.
        ...
        #The forword of task-interacted attention block
        ...
        #The forward of task-aware attention block
        ...
        #The forward of task-specific head,using fc+batch normalization+LeakyRelu
...
##The implementation of Label smoothing cross entropy
    ...
    def forward(self, output, target):
        ...
        if self.reduction=='sum':
            ...
            if self.reduction=='mean':
                loss = loss.mean()
        #The result of label smoothing cross entropy
        return loss*self.eps/c + (1-self.eps) * F.nll_loss(log_preds, target, reduction=self.reduction)

##The implementation of multi-task loss,including two forms of loss: weighted and single
##The forward of KGE-UNIT
class MultiTaskLoss(nn.Module):
    ...
    def loss(self,dti_loss,ddi_loss,mode = 'weighted'):
        if mode =='weighted':
            #The implement of weighted mult-task loss.
    ...
    def forward(self,hf_dti,sf_dti_drug,sf_dti_protein,hf_ddi,sf_ddi_d1,sf_ddi_d2,dti_labels=None,ddi_labels=None,mode= 'single',eval=False):
        if not eval:
            ### The train mode:
            #The output of KGE-UNIT,including dti and ddi
            ...
            #Use nn CrossEntropyLoss output 2, training does not require softmax; adding softmax
            ddi_loss = self.criterion_2cls(ddi_pred.cuda(),ddi_labels.cuda())
            dti_loss =self.criterion_2cls(dti_pred.cuda(),dti_labels.cuda())
            #output dim is 1 when using F.binary_ Cross_ Entropy 
            ...
            #The output of KGE-UNIT,including dti and ddi
            dti_pred,ddi_pred = self.tasks(hf_dti,sf_dti_drug,sf_dti_protein,hf_ddi,sf_ddi_d1,sf_ddi_d2)
            #Use nn CrossEntropyLoss output 2, training does not require softmax; Testing requires adding softmax
