...
def train(i,model,use_pro,patience):
    ...
    result = pipeline(training=training,testing = testing,
                       model=KGE_NAME,device='gpu',model_kwargs=dict(embedding_dim =400),
                                                                         ...
                                                                         #stopper = 'early',
                                                                         #stopper_kwargs= {"frequency":25,"patience":4,"relative_delta":0.005,"metric":"hits@10"},
                                                                         )
    en_re = result.model.entity_representations[0](indices=None)
    
    if args.save:
        if not os.path.exists(os.path.join(best_model_path_kge,args.scenario)):
            os.makedirs(os.path.join(best_model_path_kge,args.scenario))
        result.save_to_directory(os.path.join(best_model_path_kge,args.scenario,str(i)))
    ...
    ######ampligraph heterogeneous features
    ...
    for ep in range(args.epochs):
        ...
        loss_ddi_e = 0
        loss_total_e = 0
        for idx_data,(hf_dti,sf_dti_d,sf_dti_p,hf_ddi,sf_ddi_d1,sf_ddi_d2,label_dti,label_ddi) in enumerate(train_generator):
    ...
    with torch.set_grad_enabled(False):
        ...
        if args.save:
            if not os.path.exists(os.path.join(best_model_path_kgeunit,args.scenario)):
                os.makedirs(os.path.join(best_model_path_kgeunit,args.scenario))
            torch.save(model,
                      os.path.join(best_model_path_kgeunit,args.scenario,'KGE-UNIT_'+str(i)+".pth"))
